...
- `pnpm catalog:sync-oss-fund` — regenerate OSS.Fund import from `oss-fund/directory` (preserves `firstImportedAt` per id; bumps `lastUpdatedAt` when source content hash changes; hashes stored in `ossFundSeedMeta.contentHashes`)
